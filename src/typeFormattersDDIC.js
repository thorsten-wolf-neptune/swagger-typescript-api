  ...
  [SCHEMA_TYPES.OBJECT]: (content) =>
    _.map(content, (part) => {
      ...
      switch (raw?.type) {
        case "array":
          if (raw.items["$ref"]) {
            comp = getComponentByRef(raw.items["$ref"]);
          }
          break;
        default:
          if (raw["$ref"]) {
            comp = getComponentByRef(raw["$ref"]);
          }
          break;
      }

      if (comp?.isDDIC) {
        result = `${extraSpace}${part.fieldDDIC};\n`;
      } else {
        result = `${extraSpace}${part.field};\n`;
      }
...
const inlineExtraFormattersDDIC = {
  [SCHEMA_TYPES.OBJECT]: (parsedSchema) => {
    return {
      ...
      content: _.isString(parsedSchema.content)
        ...
        ? `{ ${parsedSchema.content.map((part) => part.fieldDDIC).join(", ")} }`
        : TS_KEYWORDS.OBJECT,
    };
  },
  [SCHEMA_TYPES.ENUM]: (parsedSchema) => {
    return {
      ...
      content: parsedSchema.$ref
        ...
        : _.uniq(
            _.compact([
              ..._.map(parsedSchema.contentDDIC, ({ value }) => `${value}`),
